...
public class ZstandardDecompressor implements Decompressor {
  ...
  public synchronized int decompress(byte[] buffer, int off, int len) throws IOException {
    ...
    if (!outputBuffer.hasRemaining()) {
      ...
      // TODO: Hack to hardcode 10x!!!
      int decompressedSize = inputBuffer.limit() * 10;
      if (decompressedSize > outputBuffer.capacity()) {
        outputBuffer = ByteBuffer.allocate(decompressedSize);
      }
  ...
  public synchronized void setInput(byte[] buffer, int off, int len) {
    ...
    if (inputBuffer.capacity() - inputBuffer.position() < len) {
      ByteBuffer newBuffer = ByteBuffer.allocate(inputBuffer.position() + len);
      inputBuffer.rewind();
