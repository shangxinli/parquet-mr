...
public class TestTransCompressionCommand {
  ...
  private void validColumnIndex(String inputFile, String outFile) throws Exception {
    ...
    try (CompressionConverter.TransParquetFileReader inReader = new CompressionConverter.TransParquetFileReader(HadoopInputFile.fromPath(new Path(inputFile), conf), HadoopReadOptions.builder(conf).build());
         CompressionConverter.TransParquetFileReader outReader = new CompressionConverter.TransParquetFileReader(HadoopInputFile.fromPath(new Path(outFile), conf), HadoopReadOptions.builder(conf).build())) {
    ...
    while (readValues < totalChunkValues) {
      ...
      switch (pageHeader.type) {
        case DICTIONARY_PAGE:
          DictionaryPageHeader dictionary_page_header = pageHeader.dictionary_page_header;
